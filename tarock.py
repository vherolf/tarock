...
* “Save Mapping” writes the mapping to *mapping.csv*.
...
class MainWindow(QWidget):
    ...
    MAP_FILE = Path("mapping.csv")

    def __init__(self) -> None:
        super().__init__()
        self.setWindowTitle("Table & Round – 4 names side‑by‑side")
    ...
    def _build_ui(self) -> None:
        ...
        # 20 rows × 2 columns (Number | Name)
        self.mapping_edits: list[tuple[QLineEdit, QLineEdit]] = []   # (number_le, name_le)
        self.mapping: dict[int, str] = {}                            # number → name

        for row in range(20):
            # Number column ----------------------------------------------------
            num_le = QLineEdit()
            num_le.setPlaceholderText(f"№ {row+1}")          # “№ 1”, “№ 2”, …
        ...
        form_layout.addWidget(QLabel("Table Number:"), 0, 0)
        form_layout.addWidget(self.table_edit, 0, 1)

        # ----- Round -----
        self.round_spin = QSpinBox()
        self.round_spin.setRange(1, 1000)
        self.round_spin.setValue(1)
        form_layout.addWidget(QLabel("Round:"), 0, 2)
        form_layout.addWidget(self.round_spin, 0, 3)
        ...
        for i in range(4):
            name_le = QLineEdit()
            name_le.setPlaceholderText(f"Name {i + 1}")
            points_sb = QSpinBox()
            points_sb.setRange(-100, 100)
            points_sb.setValue(0)

            self.name_edits.append(name_le)
            self.points_spins.append(points_sb)

            # Name – first row
            form_layout.addWidget(name_le, 1, i)
    ...
    def _save_mapping_to_file(self):
        """Write the current number→name pairs to *mapping.csv*."""
        mapping = {}
        ...
        try:
            ...
            self.status_lbl.setText(
                f"Mapping saved – {len(mapping)} pairs written to mapping.csv"
            )
        except OSError as exc:
            self.status_lbl.setText(f"Could not write mapping.csv: {exc}")

    # ------------------------------------------------------------------
    def _load_mapping_from_file(self):
        """Read *mapping.csv* (if it exists) and pre‑populate the fields."""
        ...
        try:
            ...
            self.status_lbl.setText(f"Could not read mapping.csv: {exc}")
    ...
    def _clear_fields(self) -> None:
        ...
        self.round_spin.setValue(1)
        for le, sb in zip(self.name_edits, self.points_spins):
            le.clear()
            sb.setValue(0)
    ...
    def _on_submit(self) -> None:
        ...
        names = [le.text().strip() for le in self.name_edits]
        points = [sb.value() for sb in self.points_spins]

        if any(not n for n in names):
            self.status_lbl.setText("All four name fields must be filled.")
            return
        ...
        entry = {
            ...
            "round": round_number,
            "people": [
                {"name": name, "points": pt} for name, pt in zip(names, points)
            ],
        }
    ...
    def _populate_form(self, entry: dict) -> None:
        ...
        for le, sb, person in zip(
            self.name_edits, self.points_spins, entry["people"]
        ):
    ...
    def _entries_to_csv(self) -> str:
        ...
        header = (
            "Table,Round,"
            + ",".join(f"Name{i+1},Points{i+1}" for i in range(4))
        )
        rows = [header]
    ...
    def _load_from_file(self) -> None:
        ...
        try:
            ...
            header = rows[0]
            for row in rows[1:]:
                # Expected format: Table,Round,Name1,Points1,Name2,Points2,...
                table = int(row[0])
                round_ = int(row[1])
                people = []
                for i in range(4):
    ...
    def _on_sum_and_rank(self) -> None:
        ...
        print(ranking_text, flush=True)

        # Optionally write to a file for later reference
        try:
            with open("ranking.txt", "w", encoding="utf-8") as f:
                f.write(ranking_text)
        except OSError as exc:
            self.status_lbl.setText(f"Could not write ranking.txt: {exc}")

        # Give the user a visual hint that the operation succeeded
        self.status_lbl.setText("Ranking generated – see console / ranking.txt.")
