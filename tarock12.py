...
class MainWindow(QWidget):
    ...
    def _on_submit(self) -> None:
        ...
        entry = {
            ...
            "round": round_number,
            "people": [
                {"playernumber": name, "points": pt} for name, pt in zip(player_numbers, points)
            ],
        }
    ...
    def _display_current_card(self) -> None:
        ...
        for i, person in enumerate(entry["people"], start=1):
            lines.append(f"{person['playernumber']} – Points: {person['points']}")
    ...
    def _entries_to_csv(self) -> str:
        ...
        for entry in self.entries:
            ...
            for person in entry["people"]:
                values.extend([person["playernumber"], str(person["points"])])
    ...
    def _on_sum_and_rank(self) -> None:
        """
        Aggregate points for each player number across all entries,
        ...
        # Dictionary: {player_number: total_points}
        totals: dict[str, int] = {}

        for entry in self.entries:
            for person in entry["people"]:
                player_number = person["playernumber"]
                pts = person["points"]
                totals[player_number] = totals.get(player_number, 0) + pts
        ...
        for rank, (player_number, total) in enumerate(sorted_totals, start=1):
            # Get player name from mapping if available, otherwise use player number
            player_name = self.mapping.get(int(player_number), player_number) if player_number.isdigit() else player_number
            lines.append(f"{rank},{player_number},{player_name},{total}")
